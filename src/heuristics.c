...
//     memmove(sol.tour, inst->best_sol, (inst->nnodes + 1) * sizeof(double));
//     if (VERBOSE >= 50){
//         printf("----------------------------------------------------------------------------------------------\n\n");
...
//         print_error("Memory allocation failed");
//     }
//     memmove(best_sol.tour, sol.tour, (inst->nnodes + 1) * sizeof(double));
//     double best_tour_cost = inst->best_sol_cost;
...
//         double running_tour_cost = inst->best_sol_cost;
//         if(running_tour_cost >= best_tour_cost-EPSILON) {
//             memmove(sol.tour, best_sol.tour, (inst->nnodes + 1) * sizeof(double));
//             for(int i = 0; i < ((int)jumps_to_perform % max_jumps); i++) {
//                 three_opt(sol.tour, inst);
//             }
...
void two_opt(solution *sol, instance *inst) {
    ...
    // Initialize the tour with the best solution
    memcpy(temp_sol, sol, (inst->nnodes + 1) * sizeof(double));
    if (VERBOSE >= 60){
                ...
                if (cost_delta < -EPSILON) {
                    ...
                    if (inst->best_sol->tour_cost == temp_sol->tour_cost) {
                        ...
                        if (time_now-inst->start_time > inst->time_limit) {
                            ...
                            memcpy(sol, temp_sol, (inst->nnodes+1) * sizeof(double));
                            free(temp_sol); 
                            return; 
                        }
                    }
