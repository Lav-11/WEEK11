...
class CmdLineParser(object):
    def __init__(self):
        self.parser = argparse.ArgumentParser(usage='usage: python3 perfprof.py [options] cvsfile.csv outputfile.pdf')
        # default options
        self.parser.add_argument("-D", "--delimiter", dest="delimiter", default=None, help="delimiter for input files")
        self.parser.add_argument("-M", "--maxratio", dest="maxratio", default=4, type=float, help="maxratio for perf. profile")
        self.parser.add_argument("-S", "--shift", dest="shift", default=0, type=float, help="shift for data")
        self.parser.add_argument("-L", "--logplot", dest="logplot", action="store_true", default=False, help="log scale for x")
        self.parser.add_argument("-T", "--timelimit", dest="timelimit", default=1e99, type=float, help="time limit for runs")
        self.parser.add_argument("-P", "--plot-title", dest="plottitle", default=None, help="plot title")
        self.parser.add_argument("-X", "--x-label", dest="xlabel", default='Time Ratio', help="x axis label")
        self.parser.add_argument("-B", "--bw", dest="bw", action="store_true", default=False, help="plot B/W")
